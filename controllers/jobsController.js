...
const getAllJobs = async (req, res) => {
  ...
  if (status !== 'all') {
    defaultQuery.status = status;
  }
  if (jobType !== 'all') {
    defaultQuery.jobType = jobType;
  }
  if (search) {
    defaultQuery.position = { $regex: search, $options: 'i' };
  }

  // No await. Need to chain while still a promise
  let result = Job.find(defaultQuery);
  ...
  const totalJobs = await Job.countDocuments(defaultQuery);
